...
function getTransporter() {
  ...
  if (!smtpEmail || !smtpPass) return null;
...
export async function sendPasswordResetEmail({
  ...
  if (!transporter || !smtpEmail) {
    console.log(`\n========================================`);
    console.log(`[DIGI VIP Password Reset Code]`);
    console.log(`To: ${to} (${name})`);
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset Your DIGI VIP Password</title>
</head>
<body style="margin: 0; padding: 0; background-color: #0b0719; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #f3f0ff;">
  <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color: #0b0719; padding: 40px 20px;">
    <tr>
        ...
        <table width="100%" max-width="540" style="max-width: 540px; background-color: #130e29; border: 1px solid rgba(139, 92, 246, 0.25); border-radius: 24px; padding: 36px 32px; box-shadow: 0 20px 50px rgba(0,0,0,0.5);">
          ...
          <tr>
            <td align="center" style="padding-bottom: 28px;">
              <p style="margin: 0; font-size: 15px; color: #a99fc6; line-height: 1.6;">
                Hi <strong style="color: #ffffff;">${name}</strong>,<br>
                We received a request to reset your DIGI VIP account password. Use the 6-digit verification code below to set a new password.
              </p>
            </td>
          </tr>
          ...
          <tr>
            <td align="center" style="padding-bottom: 28px;">
              <div style="background: linear-gradient(135deg, rgba(239, 68, 68, 0.15), rgba(124, 58, 237, 0.15)); border: 2px dashed #a855f7; border-radius: 16px; padding: 18px 32px; display: inline-block;">
                <span style="font-family: 'Courier New', Courier, monospace; font-size: 36px; font-weight: 900; letter-spacing: 8px; color: #c084fc;">
                  ${code}
                </span>
              </div>
            </td>
          ...
          <tr>
            <td align="center" style="padding-bottom: 32px;">
              <p style="margin: 0; font-size: 13px; color: #fbbf24;">
                ⏳ This password reset code will expire in <strong>10 minutes</strong>.
              </p>
              <p style="margin: 8px 0 0 0; font-size: 12px; color: #6d6488;">
                If you did not request a password reset, please ignore this email or change your password immediately.
              </p>
            </td>
          </tr>
  ...
  const info = await transporter.sendMail({
    from: `"DIGI VIP Security" <${smtpEmail}>`,
    to,
    subject: `${code} is your DIGI VIP password reset code`,
    html,
  });
